...
/**
 * Transform GFA 1.0 links in Parquet format to property graph CSV format.
 */
object LinksToPropertyGraph {
  val logger = Logger("com.github.heuermh.adam.gfa.LinksToPropertyGraph")
  ...
  def main(args: Array[String]) {
    ...
    val conf = new SparkConf()
      .setAppName("Transform Graphical Fragment Assembly (GFA) 1.0 links in Parquet format to property graph CSV format.")
    ...
    val links = spark.sql("select id, source.id as sourceId, target.id as targetId, source.orientation as sourceOrientation, target.orientation as targetOrientation, recordType as interaction, overlap, mappingQuality, mismatchCount from parquet.`" + args(0) + "`")
    ...
    renamedLinks.write.option("header", true).csv(args(1) + "-edges.csv")
